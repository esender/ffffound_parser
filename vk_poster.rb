...
db.execute( "select * from images WHERE id IS 44 ORDER BY created_at ASC LIMIT 10" ) do |row|
	if row[1].include? gif
    agent.get(row[1]).save 'temp/pic.gif'
    temp_pic = 'temp/pic.gif'
    url = vk.docs.get_wall_upload_server(group_id: 69899944)
    p url
    #upl = VkontakteApi.upload(url: url[:upload_url], photo: [temp_pic, 'image/gif'])
    #p upl
    #post_item = vk.docs.save(upl.merge({gid:69899944}))
    #vk.wall.post(owner_id: -69899944, attachments: post_item.map(&:id).join(','))
  else
    #agent.get(row[1]).save 'temp/pic.jpg'
    #temp_pic = 'temp/pic.jpg'
    #url = vk.photos.get_wall_upload_server(group_id: 69899944)
    #upl = VkontakteApi.upload(url: url[:upload_url], photo: [temp_pic, 'image/jpeg'])
    #post_item = vk.photos.save_wall_photo(upl.merge({gid:69899944}))
    #vk.wall.post(owner_id: -69899944, attachments: post_item.map(&:id).join(','))
  end
...
#url = vk.docs.get_wall_upload_server(group_id: 69899944)
#upl = VkontakteApi.upload(url: url[:upload_url], photo: [temp_pic, 'image/gif'])
#p post_item = vk.docs.save_wall_photo(upl.merge({gid:69899944}))
#vk.wall.post(owner_id: -69899944, attachments: post_item.map(&:id).join(','))